...
#include<sys/time.h> // requred for struct timeval
#include<chrono>
...
Server::Server(int port): port(port),server_fd(-1),threadPool(4),should_exit(false){
}
...
Server::~Server(){
	if(server_fd != -1){
		close(server_fd); // destroys the kernal socket structure now server_fd does not represent a valid socket
...
void Server::stop(){
	should_exit = true;
	if(server_fd != -1){
		shutdown(server_fd,SHUT_RD); // shutdown disables the socket but does not destroy it
                                             // so server_fd still represents a kernel socket structure, but this structure
                                             // is broken and calling accept() on it will return an error code or something
	}
}
...
void Server::handle_client(int client_fd){
	...
	str_stream << std::this_thread::get_id();

        log("Worker [" + str_stream.str() + "] is handling FD: " + std::to_string(client_fd));

	// Simulate heavy work for 5 seconds to allow other workers to get a chance
    	// std::this_thread::sleep_for(std::chrono::seconds(5));

    	// 2. Set a 3-second timeout on the socket
	// avoids the read function from waiting forever
...
void Server::run(){
    ...
    while(!should_exit){
        ...
        if(client_fd < 0){
	    if(should_exit){
		log("Server stopping (shutdown signal received)...");
		break;
	    }
	...
	std::stringstream ss;
        ...
        log("Worker [" + ss.str() + "] is handling FD: " + std::to_string(client_fd));

	*/

        // FIX 4: Correct Thread Syntax for Member Function
	threadPool.enqueue([this,client_fd]{
		this->handle_client(client_fd);
	});
    }
    log("Server loop exited. Cleaning up...");
}
